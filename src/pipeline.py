...
def run_pipeline(spark, cfg: PipelineConfig) -> None:
    ...
    curated_failures_df = _empty_like(failing_curated_candidates[0])
    for candidate in failing_curated_candidates:
        curated_failures_df = curated_failures_df.unionByName(candidate, allowMissingColumns=True)

    # One enriched row per review_id can trigger multiple rules; keep a single record to avoid over-counting.
    curated_failures_df = curated_failures_df.dropDuplicates(["review_id"])

    # Keep all duplicate physical rows so source_count == curated_count + quarantined_count remains true.
    quarantine_df = curated_failures_df.unionByName(duplicate_quarantine, allowMissingColumns=True)
