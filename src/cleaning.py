...
def clean_and_normalize(df: DataFrame) -> DataFrame:
    ...
    if "verified" in cleaned.columns:
        cleaned = cleaned.withColumn("verified_purchase", F.coalesce(F.col("verified").cast("boolean"), F.lit(False)))
    else:
        cleaned = cleaned.withColumn("verified_purchase", F.lit(False))
